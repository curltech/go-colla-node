...
func (this *WebrtcSimplePeer) Create(targetPeerId string, iceServer []webrtc.ICEServer, initiator bool, options *webrtc2.WebrtcOption, router *simplepeer.Router) {
	...
	if this.router != nil && this.router.RoomId != "" {
		...
		if !ok {
			golog.Errorf("RegistEvent %v fail", webrtc2.EVENT_CONNECT)
		}
		...
		if !ok {
			golog.Errorf("RegistEvent %v fail", webrtc2.EVENT_CLOSE)
		}
	}
	/**
	 * 可以发起信号
	...
	this.simplePeer.RegistEvent(webrtc2.EVENT_SIGNAL, func(evt *webrtc2.PeerEvent) (interface{}, error) {
		golog.Infof("can signal to peer:%v;connectPeer:%v;session:", this.TargetPeerId, this.ConnectPeerId, this.ConnectSessionId)
	...
	this.simplePeer.RegistEvent(webrtc2.EVENT_CONNECT, func(evt *webrtc2.PeerEvent) (interface{}, error) {
		golog.Infof("connected to peer:%v;connectPeer:%v;session:%v, can send message", this.TargetPeerId, this.ConnectPeerId, this.ConnectSessionId)
		this.end = time.Now()
		golog.Infof("connect time:%v", (this.end.Unix() - this.start.Unix()))
	...
	this.simplePeer.RegistEvent(webrtc2.EVENT_CLOSE, func(evt *webrtc2.PeerEvent) (interface{}, error) {
		golog.Infof("connected to peer:%v;connectPeer:%v;session:%v, is closed", this.TargetPeerId, this.ConnectPeerId, this.ConnectSessionId)
	...
	this.simplePeer.RegistEvent(webrtc2.EVENT_DATA, func(evt *webrtc2.PeerEvent) (interface{}, error) {
		data := evt.Data.([]byte)
		golog.Infof("got a message from peer:%v", string(data))
...
func (this *WebrtcSimplePeer) Send(data []byte) error {
	if this.simplePeer.Connected() {
		...
		golog.Errorf("peerId:%v;connectPeer:%v session:%v  webrtc connection state is not connected", this.TargetPeerId, this.ConnectPeerId, this.ConnectSessionId)
	}
