...
func (this *PbftConsensus) ReceiveConsensus(chainMessage *entity3.ChainMessage) (*entity3.ChainMessage, error) {
	...
	if peerIds != nil && len(peerIds) > 2 {
		...
		for _, peerId := range peerIds {
			...
			// 封装消息，异步发送
			go action.ConsensusAction.ConsensusDataBlock(peerId, msgtype.CONSENSUS_PBFT_PREPREPARED, dataBlock, "")
		}
	} else {
...
func (this *PbftConsensus) ReceivePreprepared(chainMessage *entity3.ChainMessage) (*entity3.ChainMessage, error) {
	...
	if peerIds != nil && len(peerIds) > 2 {
		for _, id := range peerIds {
			...
			go action.ConsensusAction.ConsensusLog(id, msgtype.CONSENSUS_PBFT_PREPARED, log, "")
		}
	} else {
		logger.Sugar.Errorf("LessPeerLocation")
...
func (this *PbftConsensus) ReceivePrepared(chainMessage *entity3.ChainMessage) (*entity3.ChainMessage, error) {
	...
	* 检查准备消息来源
	...
	if peerIds != nil && len(peerIds) > 2 {
		...
		f := len(peerIds) / 3
		logger.Sugar.Infof("findCountBy current status:%v;count:%v", msgtype.CONSENSUS_PBFT_PREPARED, count)
		// 收到足够的数目
		if count > 2*f {
			log.PeerId = myPeerId
			...
			if cacheLog == nil {
				...
				for _, id := range peerIds {
					...
					go action.ConsensusAction.ConsensusLog(id, msgtype.CONSENSUS_PBFT_COMMITED, log, "")
				}
			}
		}
	} else {
...
func (this *PbftConsensus) ReceiveCommited(chainMessage *entity3.ChainMessage) (*entity3.ChainMessage, error) {
	...
	if peerIds != nil && len(peerIds) > 2 {
		...
		f := len(peerIds) / 3
		logger.Sugar.Infof("findCountBy current status:%v;count:%v", msgtype.CONSENSUS_PBFT_COMMITED, count)
		if count > 2*f {
			/**
			 * 数据块记录有效
			 */
